...
export default function Page({ params }: { params: { internship_id: string } }) {
  ...
  async function ApplyForInternship() {
    try {
      ...
      console.log(internship?.jobID, internship?.username, internship?.company, internship?.title, internship?.description, internship?.location);
      if (access_token && internship?.jobID && internship?.username && internship?.company && internship?.title && internship?.description && internship?.location) {
        console.log("inside");
