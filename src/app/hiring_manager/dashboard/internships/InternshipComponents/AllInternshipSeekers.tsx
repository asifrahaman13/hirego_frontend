...
export default function AllInternshipSeekers() {
  ...
  useEffect(() => {
    async function fetchInternships() {
      ...
      if (access_token) {
        const response = await internshipInterface.getAllJobPosting(access_token);
    ...
    <div className="flex w-screen h-screen   flex-row bg-gray-50">
      <div className="w-3/4 flow-root overflow-y-scroll h-full no-scrollbar">
        <div className="-mx-4 -my-2 w-full overflow-x-auto sm:-mx-6 px-12 lg:-mx-8">
          <div className="text-xl m-4 font-semibold font-sans">My posted internships</div>
          <div className="flex flex-col w-full gap-4">
            {internships.map((person) => (
              <>
                <Link href={`/hiring_manager/dashboard/internships/${person.jobID}`} className="bg-white w-full border gap-4 flex flex-col rounded-lg px-4 py-4">
